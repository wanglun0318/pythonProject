...
class MyGridTable(wx.grid.GridTableBase):
    def __init__(self):
        super().__init__()
        self.colLabels = column_names    # 表格初始化

    def GetNumberRows(self):
        return len(data)                 # 返回网格行数

    def GetNumberCols(self):
        return len(data[0])              # 返回网格列数

    def GetValue(self, row, col):
        return data[row][col]            # 返回单元格内容

    def GetColLabelValue(self, col):
        return self.colLabels[col]       # 返回列标题
...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        self.Center()  # 窗口居中
        self.grid = self.CreateGrid(self)      # 调用方法创建网格对象
    ...
    def OnLabelLeftClick(self, event):
        print('RowIdX：{0}'.format(event.GetRow()))     # 获取行
        print('ColIdX：{0}'.format(event.GetCol()))     # 获取选中列索引
        print(data[event.GetRow()])

        # 该语句放在时间处理的最后一行，可以确保继续处理其他事件
        event.Skip()
