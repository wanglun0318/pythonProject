...
- 菱形继承/钻石继承问题
    - 多个子类继承子同一个父类，这些子类由被同一个类继承。于是继承关系图形参一个菱形谱
    - 关于多继承的MRO[https://www.cnblogs.com/whatisfantasy/p/6046991.html]
        - MRO就是多继承中，用于保存继承顺序的一个列表
        - Python本身采用C3算法来多继承的菱形继承进行计算结果
        - MRO列表的计算原则：
            - 子类永远在父类前面
            - 如果多个父类，则根据继承法中括号内类的书些顺序存放
...
- Mixin设计模式
    - 主要采用多继承方式对类的功能进行扩展
        ...
        - Mixin & MRO[https://blog.csdn.net/robinjwong/article/details/48375833]
        - Mixin模式[https://blog.csdn.net/robinjwong/article/details/48375833]
    ...
    - 优点
        ...
        - 可以方便的组织和维护不同功能组件的划分
        - 可以根据需要任意调整功能类的组合
        - 可以避免创建很多新的类，而导致类的继承混乱
        
        
# 4 类相关函数
    - issubclass  检测一个类是否是另一个类的子类
    - isinstance  检测一个对象是否是另个类的实例
    - hasattr     检测一个对象是否是成员XXX
...
# 6. 类的内置属性
    ...
    - __doc__：获取类的文档信息
    - __name__:获取类名，如果在模块中使用，获取模块名称
    - __bases__:获取当前类的所有父类，以元组的方式显示
...
# 8. 类和对象的三种方法
- 实例方法
    - 需要实例话对象才能使用的方法，使用过程中可能需要截止对象的其他对象的方法完成
- 静态方法
    - 不需要实例化，通过类直接访问
- 类方法
    - 不需要实例化，类方法的第一个参数，一般命名为cls
